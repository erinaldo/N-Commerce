...
namespace Infraestructura.Repositorio
{
    ...
    {
        public CuentaCorrienteRepositorio(DataContext context)
            : base(context)
        {
        }
        ...
        public override MovimientoCuentaCorriente Obtener(long entidadId, string propiedadNavegacion = "")
        {
            var resultado = propiedadNavegacion.Split(new[] {','},
                ...
                IQueryable<MovimientoCuentaCorriente>>(_context.Set<Movimiento>().OfType<MovimientoCuentaCorriente>(),
                (current, include) => current.Include(include));

            return resultado.FirstOrDefault(x => x.Id == entidadId);
        }

        public override IEnumerable<MovimientoCuentaCorriente> Obtener(
            Expression<Func<MovimientoCuentaCorriente, bool>> filtro = null, string propiedadNavegacion = "")
        {
