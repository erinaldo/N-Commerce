...
namespace Infraestructura.Repositorio
{
    public class FacturaRepositorio : Repositorio<Factura>, IFacturaRepositorio
    {
        ...
        public Factura Obtener(long entidadId, string propiedadNavegacion = "")
        {
            var resultado = propiedadNavegacion.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Aggregate<string, IQueryable<Factura>>(_context.Set<Comprobante>().OfType<Factura>(),
                    (current, include) => current.Include(include));

            return resultado.FirstOrDefault(x => x.Id == entidadId);
        }

        public IEnumerable<Factura> Obtener(Expression<Func<Factura, bool>> filtro = null, string propiedadNavegacion = "")
        {
