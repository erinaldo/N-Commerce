...
{
    public class CtaCteClienteRepositorio : Repositorio<CuentaCorrienteCliente>, ICtaCteClienteRepositorio
    {
        public CtaCteClienteRepositorio(DataContext context):base(context)
        {
        }
        ...
        public override CuentaCorrienteCliente Obtener(long entidadId, string propiedadNavegacion = "")
        {
            var resultado = propiedadNavegacion.Split(new[] {','}, StringSplitOptions.RemoveEmptyEntries)
                .Aggregate<string, IQueryable<CuentaCorrienteCliente>>(
                    _context.Set<Comprobante>().OfType<CuentaCorrienteCliente>(),
        ...
        public override IEnumerable<CuentaCorrienteCliente> Obtener(Expression<Func<CuentaCorrienteCliente, bool>> filtro = null, string propiedadNavegacion = "")
        {
            ...
            var resultado = propiedadNavegacion.Split(new[] {','}, StringSplitOptions.RemoveEmptyEntries)
                .Aggregate<string, IQueryable<CuentaCorrienteCliente>>(resultadoClient,
                    (current, include) => current.Include(include));
